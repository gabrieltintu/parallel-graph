...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	while (tp->first_added == 0)
		pthread_cond_wait(&tp->cond_dequeue, &tp->mutex);
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	if (pthread_cond_init(&tp->cond, NULL) != 0) {
		perror("Failed to initialize cond");
		exit(EXIT_FAILURE);
	}

	if (pthread_cond_init(&tp->cond_dequeue, NULL) != 0) {
		perror("Failed to initialize cond");
		exit(EXIT_FAILURE);
	}
	tp->first_added = 0;
